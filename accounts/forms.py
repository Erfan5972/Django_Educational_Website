...
class UserRegistrationForm(forms.Form):
    phone_number = PhoneNumberField(region='IR', widget=forms.TextInput(attrs={'class': 'sign_in_input', 'placeholder': 'شماره تلفن', 'id': 'id_phone_number', 'required': True}))
    full_name = forms.CharField(max_length=500, widget=forms.TextInput(attrs={'class': 'sign_in_input', 'placeholder': 'نام و نام خانوادگی', 'id': 'id_full_name', 'required': True}))
    password1 = forms.CharField(label='password', widget=forms.PasswordInput(attrs={'class': 'sign_in_input', 'placeholder': 'رمز عبور', 'id': 'id_password1', 'required': True}))
    password2 = forms.CharField(label='confirm password', widget=forms.PasswordInput(attrs={'class': 'sign_in_input', 'placeholder': 'تایید رمز عبور', 'id': 'id_password2', 'required': True}))
...
class UserLoginForm(forms.Form):
    phone_number = PhoneNumberField(region='IR', widget=forms.TextInput(attrs={'class': 'sign_in_input', 'placeholder': 'شماره تلفن', 'id': 'id_phone_number', 'required': True}))
    password = forms.CharField(label='password', widget=forms.PasswordInput(attrs={'class': 'sign_in_input', 'placeholder': 'رمز عبور', 'id': 'id_password1', 'required': True}))
